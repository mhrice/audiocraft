        ...
        for batch in lp:
            ...
            if self.cfg.generate.lm.unprompted_samples:
                ...
                for i, audio_f in enumerate(gen_unprompted_audio):
                    prompt_summary = attributes[i].text["description"][:50].replace(" ", "_").replace("\n", "_").replace("\"", "")
            ...
            if self.cfg.generate.lm.prompted_samples:
                ...
                for i, audio_f in enumerate(gen_audio):
                    self.log_audio(generate_stage_name, f'prompted_audio_{i}', audio_f, self.cfg.sample_rate)
                sample_manager.add_samples(
